...
```
[master b80dea8] First working version of catbind function
...
This changed two files, `pyrpoject.toml` (which we printed above) and `poetry.lock` (a record of all the packages and exact versions of them that poetry downloaded for this project). These changes are important for our package, so let's commit them to version control as well:
...
```
[master f5c146d] added pandas as a dependency
...
For your collaborators (other people, and most importantly yourself in the future) we would like to have readable and accessible documentation expressing how to install your package, and how to use the user-facing functions within it. The Python packaging ecosystem has a tool to help you make this process more efficient - [Sphinx](https://docs.readthedocs.io/en/stable/intro/getting-started-with-sphinx.html). In the Cookiecutter template we provided you to set-up this package, there is a basic docs template that the Cookiecutter progam filled in with the information you entered interactively when you ran `cookiecutter https://github.com/UBC-MDS/cookiecutter-ubc-mds.git`. These files live in the `doc` directory and are `.rst` (reStructuredText markup language) filetype. This is a lightweight markup language that works similar to Markdown but uses different syntax. The templates provided to you here are fairly well formatted already, so you do not have to change the RST formatting, however if you are interested in doing so, you can see this [CheatsSheet](https://thomas-cokelaer.info/tutorials/sphinx/rest_syntax.html) to get started.
...
> Note the use of `--dev` to specify a development, rather than a package function dependency. If you look in `pyproject.toml` you will see that `sphinx` gets added under the `[tool.poetry.dev-dependencies]` section as opposed to the `[tool.poetry.dependencies]` section (where package function dependencies get installed, such as pandas in this example).
...
> Note 1: we append `poetry run` infront of most of our commands in this Python package workflow so that we ensure we are using the software tools installed in the `poetry` virtual environment.

> Note 2: Expect many red warnings, these can be ignored and are meerely suggestions on how to improve your docs if you wish.
...
<img src="img/sphinx-docs1.png" width=600>
...
> Note - to use this, we also have to add `extensions = ['sphinx.ext.napoleon']` in the `conf.py` file in the `docs` directory, but we have taken care of this for you with our Cookiecutter template.

Then we can `sphinx-apidoc`, followed by re-rending of the docs directory to complete the rendering of the package function (i.e., module) docs:
...
<img src="img/sphinx-docs3.png" width=600>

And we can click on that to see the docs for `foocat.foocat.catbind`. Which should look roughly like this:

<img src="img/sphinx-docs4.png" width=600>

Another hurray! 🎉🎉🎉 Let's commit this to version control to:
...
```
[master ae68b4e] generated and render docs for local viewing
...
0. Push your local changes to GitHub.com via `git push`.
1. visit <https://readthedocs.org/> and click on "Sign up".
2. On the following page, select "Sign up with GitHub".
3. Click "Import a Project".
4. Click "Import Manually".
5. Fill in the project details (package name and Repository URL, leave the rest as is) and click "Next".
6. Click "Build version".

After following the steps above, your docs should get successfully built on [Read the Docs](https://readthedocs.org/) and you should be able to access them via the "View Docs" button on the build page, or from the link that Cookiecutter created for your on your repositories `README.md` file.

> Note: for [Read the Docs](https://readthedocs.org/) to work with the `poetry` package workflow you need to have a `.readthedocs.yml` in the root of your Python package. We have created this for you using Cookiecutter and you can view it [here](https://github.com/UBC-MDS/cookiecutter-ubc-mds/blob/master/%7B%7Bcookiecutter.project_slug%7D%7D/.readthedocs.yml).
...
> Note - given that we use `pd.Categorical` to create objects to test on, we have to import the `pandas` package in our test file.
...
Then to run the tests, we use `poetry` to run `pytest` in our virtual environment:
...
```
[master 6fe4b6c] added unit tests for catbind
...
To github.com:ttimbers/foocat.git
   154aacc..6fe4b6c  master -> master
```

## Building your package & publishing to testPyPI {#build-n-publish}

Python packages are generally shared via the [PyPI package Index](https://pypi.org/). However, when we are just starting to develop packages, and/or at the development stage of our package, we typically first check that everything works by submitting to [testPyPi](https://test.pypi.org/). `poetry` has a command called `publish` which we can use to do this, however it's default is to publish to PyPI. We can add this to the list of repositories `poestry` knows about via `poetry config repositories.<your_project> https://test.pypi.org/legacy/` as shown below:

```
$ poetry config repositories.foocat https://test.pypi.org/legacy/
```

Before we send our package, we first need to build it to source and wheels archives (the format that PyPI distributes it via) using `poetry build`:

```
$ poetry build
```

And, finally, now to publish to testPyPI we can use `poetry publish` (you will be prompted for your testPyPI username and password, sign up for one if you have not already done so):
