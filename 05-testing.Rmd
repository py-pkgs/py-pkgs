...
You can write as many tests as you like for your code. Ideally, your tests will cover all elements of your code. In the section [Code coverage](#pytest-coverage) we'll show you how to determine how much of your code is being "covered" by your tests. But first, let's look at some ways to better organise your tests. 

### Classes, Fixtures & Parameterizations

You can populate your test module with as many individual test functions as you like, however as the number of tests grows its helpful to organise and streamline your tests. Classes, fixtures, and parameterization are a few of the key techniques that can help.

**Classes**

Classes are a way of grouping related tests together. Typically your code will consist of multiple functions, each of which needs to be tested with multiple tests. So its often useful to create a class that groups all the tests for each individual function together. You can define a test class in your test module by creating a `class` with a name prefixd with "Test". You can then put all the test functions within this class as is shown below:
...
You may have noticed some repetition in our test code defining variables (`a`, `b` and `c`) to use in our tests. This violates the "DRY" principle and is where fixtures can come in handy. Fixtures are useful for sharing resources (primarily functions or data) with test functions. It is perhaps easiest to see a simple example utilising a fixture. Below we create the fixture `test_data()` which in this case creates a dictionary of data that we would like to use with our test functions. This fixture can be passed to our test functions as an argument in the function definition, meaning that we don't have to define our data seperately in each test.
...
If we ran this test module with `poetry run pytest`, the output would tell us that it ran two tests (well, it really ran the same test twice).For most packages you'll be creating the methods discussed in this chapter will more than suffice your needs, however we've only scratched the surface of the `pytest` iceburg and it's possible to combine classes, fixtures, parameterizations and much more to help build efficient, streamlined tests which can be useful for larger projects. 

### When to write your tests

Whether you should write your tests before you code, after you code, or somewhere in between is a topic of much debate and overall this choice may come down personal preference, experience, resource availability, and code complexity. While there's no one "correct" testing workflow for everyone, we encourage you to have a go at Test-Driven-Development (TDD) - that is, writing your tests before you code. While this may seem a little counterintuitive at first, the kind of workflow can have many benefits:
