...
`pytest` will run all files of the form `test_*.py` or `*_test.py` in the current directory and its subdirectories. Standard practice for Python packages is to place test modules in a `test` subdirectory within the root package directory. The [MDS Cookiecutter template](https://github.com/UBC-MDS/cookiecutter-ubc-mds) that we downloaded and used in [The Whole Game chapter](#whole-game) to build our `foocat` package automatically created this directory structure for us:
