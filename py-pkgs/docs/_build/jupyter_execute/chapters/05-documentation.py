...
Each of these files are described in more detail below. Note that the [Cookiecutter template](https://github.com/UBC-MDS/cookiecutter-ubc-mds) described and used in {ref}`02-cookiecutter-poetry` of {ref}`02-whole-game` automatically creates and populates these files for you.
...
def catbind(a, b):
    ...
    # Concatenating pandas categoricals can be tricky! The approach here is to coerce
    # categoricals to strings, then to Pandas series, so that they can be concatenated
    # with pd.concat(). We then re-cast the result to a pandas categorical.
    concatenated = pd.concat([pd.Series(a.astype("str")),
                              pd.Series(b.astype("str"))])
    return pd.Categorical(concatenated)
```
...
If you don't want to use the MDS Cookiecutter to build a directory structure, you can see the [Sphinx documentation](https://www.sphinx-doc.org/en/master/usage/quickstart.html#) for help getting started building documentation from scratch.
...
This cookiecutter provides you with most of the base documentation you'll want for your package but you can modify it as much as you like. While it is possible to include files in your documentation that are not `.rst` format (such as .md files) with Sphinx - things start to get a little bit hacky and we recommend sticking with the workflow described above. If you want to include files such as images in your documentation, a common approach is to create a subdirectory called `_static` in the `docs` directory within which your files are placed and referenced to.

We're not quite done yet. We still need to extract all the docstrings from our code and include them in our documentation so that they are easily accessible for users. In the `index.html` page above, if you click on the `Module Index` link under the heading “Indices and tables” at the bottom of the page you will get a “Your file was not found" message. This is because we need to render our docstrings into our documentation. To do this we can use the Sphinx extension [`napolean`](https://sphinxcontrib-napoleon.readthedocs.io/en/latest/index.html) to render our NumPy style docstrings into our documentation. To add `napoleon` as a dependency to our development environment we can use the following command:
...
Now we can change back to the root `pypkgs` directory and use `sphinx-apidoc` to render our docstrings into our documentation (the renders will appear in `docs/source/`), and then `make html` to re-render our docs:
...
```{figure} img/05-documentation/documentation-2.png
---
width: 600px
...
We can click on the `pypkgs.pypkgs` module (which contains our `catbind()` function) to have a look at our rendered docstring which should look like the following:
