...
## Using Cookiecutter & Poetry to Create a Python Package Directory Structure
...
```{tip}
Pandas comes packaged with the Anaconda distribution we installed in {ref}`01-python-setup`. However, if for some reason you don't currently have Pandas installed, you can install it with `pip` or `conda`, check out the official [Pandas documentation](https://pandas.pydata.org/docs/getting_started/install.html).
```
...
### Reading & Rendering Documentation Locally

For the users of your code (including your future self) we need to have readable and accessible documentation expressing how to install your package, and how to use the functions within it. We'll discuss documentation in detail in the chapter {ref}`05-documentation`, but for now, we will demonstrate the basic steps required to get your documentation up-and-running quickly.
...
```{note}
We append `poetry run` in front of most of our commands in this Python package workflow to ensure are commands are executed within our project's virtualenv and are using only the software tools we have specifically installed in that virtual environment.
```
...
This is because we haven't written any documentation for our package function. Let's do that now by adding a `NumPy`-style docstring to the `catbind` function in `pypkgs/pypkgs.py` as shown below (we'll discuss docstring style more in the chapter {ref}`05-documentation`):
...
### Reading & Rendering Documentation Remotely
...
Before we send our package, we first need to build it to source and wheel distributions (the format that PyPI distributes and something you'll learn more about in the next chapter {ref}`03-package-structure`) using `poetry build`:
