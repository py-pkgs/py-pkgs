...
Open this new *`ci-cd.yml`* file in an editor. We are going to set up a CI workflow that triggers when someone makes a push or pull-request to the `main` branch of our repository. To set this up, copy and paste the following text into *`ci-cd.yml`*:
...
4. Install `poetry` (action: [snok/install-poetry@v1](https://github.com/snok/install-poetry)); and, 
...
2. Checking the code coverage of our tests; and,
...
3. Upload the distributions to TestPyPI and test that the package can be installed successfully; and,
...
1. The "ci" job passes — we don't want to deploy a new version of our package if it isn't passing CI. We can specify this constraint using the `needs` keyword in GitHub Actions ([documentation](https://docs.github.com/en/actions/reference/workflow-syntax-for-github-actions#jobsjob_idneeds)); and,
...
We'll discuss steps 4-7 in the sections below.

### Automatically creating a new package version

As we saw in **Chapter 7: [Releasing and versioning]**, there's a few key steps to go through when creating a new version of your package:
...
3. Tag\index{version control!tag} a new release\index{version control!release} on GitHub; and,
...
```{r 08-token-1-fig, fig.cap = "Adding the TestPyPI API token secret to our GitHub repository.", out.width = "100%", fig.retina = 2, fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
knitr::include_graphics("../images/08-token-1.png")
```

To use the [pypa/gh-action-pypi-publish@release/v1](https://github.com/pypa/gh-action-pypi-publish) action, we can add the following step to our CD workflow. Note how we configure the action to use the token `user` method, specify `password` as the `TEST_PYPI_API_TOKEN` we just added to our repository, and we point the action to the TestPyPI repository (`repository_url: https://test.pypi.org/legacy/`).
...
```{r 08-token-2-fig, fig.cap = "Adding the PyPI API token secret to our GitHub repository.", out.width = "100%", fig.retina = 2, fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
