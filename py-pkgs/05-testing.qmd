...
**@sec-03-testing-your-package** briefly introduced testing in Python package development. This chapter now goes into more detail about how to write tests\index{tests}, different types of tests (unit tests, regression tests, integration tests), and code coverage.

## Testing workflow {#sec-05-testing-workflow}
...
This workflow is illustrated in @fig-05-test-workflow.

![The testing workflow.](images/05-test-workflow.png){#fig-05-test-workflow width="100%" fig-alt="The testing workflow."}
...
As an example, consider the structure of the `pycounts` package we developed in **Chapter 3: [The Whole Game: How to Package a Python](#sec-03-how-to-package-a-python)**:

``` {.md emphasize-lines="14-16"}
pycounts
├── .readthedocs.yml
├── CHANGELOG.md
├── CONDUCT.md
...
The file *`einstein.txt`* is a text file we created in **@sec-03-writing-tests** to use in our tests. It includes a quote from Albert Einstein:

>*"Insanity is doing the same thing over and over and expecting different results."*

The file *`test_pycounts.py`* is where the tests we want to run with `pytest` should be. That file contains the following test we wrote in **@sec-03-running-tests**, using the format expected by `pytest`, a function prefixed with `test_` that includes an `assert` statement.
...
## Writing tests {#sec-05-writing-tests}
...
A `pytest` test function can actually include multiple `assert` statements and if any of the included `assert` functions fail, the whole test will fail. As an example of a unit test with multiple `assert` statements, we'll write a new test in our *`test_pycounts.py`* file for the `plot_words()` function of our `pycounts.plotting` module. We developed the `plot_words()` function in **@sec-03-adding-dependencies-to-your-package** and show it below:
...
### Test that a specific error is raised {#sec-05-test-that-a-specific-error-is-raised}
...
We've been testing our `pycounts` package on the simple "Einstein quote" fixture, but how does it perform on real data? We added some example real data to our package in **@sec-04-including-data-in-a-package**; a *.txt* file of the novel *Flatland*, by Edwin Abbott[@abbott1884] ([available online](https://www.gutenberg.org/ebooks/97)). However, it would be impossible to count all of the words in that text by hand to come up with an "expected" result.
...
``` {.python emphasize-lines="10-13, 19-22"}
from pycounts.pycounts import count_words
from pycounts.plotting import plot_words
from pycounts.datasets import get_flatland
import matplotlib
...
``` {.python emphasize-lines="8-9, 15, 21"}
from pycounts.pycounts import count_words
from pycounts.plotting import plot_words
from pycounts.datasets import get_flatland
import matplotlib
...
Parameterizations\index{parameterization}\index{tests!parameterization} can be useful for running a test multiple times using different arguments. For example, recall in **@sec-05-test-that-a-specific-error-is-raised** that we added some code to `pycounts`'s `plot_words()` function that raises a `TypeError` if a user inputs an object other than a `Counter` object to the function. We wrote a test to check that in our *`test_pycounts.py`* file as follows:
...
``` {.python emphasize-lines="3-10, 11, 14"}
# ... rest of file hidden ...
...
## Code coverage {#sec-05-code-coverage}
...
### Line coverage {#sec-05-line-coverage}
...
### Coverage reports {#sec-05-types-of-code-coverage}
...
The report will be available at *`htmlcov/index.html`* (relative to your working directory) and will look like @fig-05-test-report-1.

![HTML test report output by pytest-cov.](images/05-test-report-1.png){#fig-05-test-report-1 width="100%" fig-alt="HTML test report."}

We can click on elements of the report, like the *`datasets.py`* module, to see exactly what lines/branches the tests are hitting/missing, as shown in @fig-05-test-report-2:

![Detailed view of the datasets module in the HTML report.](images/05-test-report-2.png){#fig-05-test-report-2 width="100%" fig-alt="Detailed view of the datasets module in the HTML report."}

## Version control {#sec-05-version-control}

Throughout this chapter, we added a significant number of tests to our *`test_pycounts.py`* file, and made a small change to our `pycounts.plotting.plot_words()` function in **@sec-05-test-that-a-specific-error-is-raised** to have it check that users pass a `Counter` object as an input argument. These changes will form part of a new release of our package that we'll develop in **Chapter 7: [Releasing and versioning](#sec-07-releasing-and-versioning)**. So, if you're following along building the `pycounts` package yourself and using version control\index{version control}, commit these changes to your local and remote repositories using the commands below. If you're not building the `pycounts` package or not using version control, you can skip to the next chapter.
