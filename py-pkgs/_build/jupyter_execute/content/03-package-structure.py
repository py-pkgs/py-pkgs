...
- {ref}`03-python-poetry` (not a "package state" but a useful tool for building/installing/managing different states)
...
Python's installer `pip` always prefers installing built distributions (`wheels`) over source distributions (`sdists`) because installation is faster. Building `wheels` is similar to building source distributions with `setuptools` as described in the previous section. We won't go into details here because for most users we recommend the use of Python poetry (described later in {ref}`03-python-poetry`)) which handles this build process for you in a simple and intuitive way. However, if you're interested in learning more about using `setuptools` to build a `wheel` of your project we recommend taking a look at the [Python Packaging User Guide tutorial](https://packaging.python.org/tutorials/packaging-projects/).
...
The previous sections gave a high level overview of Python's standard packaging options and tools. However, in {ref}`02-whole-game` we used Poetry to create a toy Python package - so where does this fit into the Python packaging landscape? Well, in the previous sections on {ref}`03-source-distributions` and {ref}`03-built-distributions`, we really only touched the tip of the iceberg of Python packaging. When creating a package there's a lot of customisation to think about with your `setup.py` file, and a host of other files we didn't even talk about (e.g., `requirements.txt`, `setup.cfg`, etc)! Needless to say, packaging in Python can be hard to understand, especially for beginners. These words echo the sentiments of poetry's creator Sébastien Eustace and the motivation for creating the tool:
...
That "single configuration file" is `pyproject.toml` (you can read more about `.toml` files [here](https://www.python.org/dev/peps/pep-0518/)). Essentially, poetry is based on all the concepts of `sdists` and `wheels` discussed previously - it just simplifies and streamlines the whole packaging process in an intuitive way. In fact, the `poetry build` command you used in {ref}`02-build-and-publish`, actually creates the `sdist` and `wheel` distributions of your package for you (depending on how you've configured your project). It really is simple to create and distribute Python packages with poetry - go back and check out {ref}`02-whole-game` for our recommended workflow, or check out the [poetry docs](https://python-poetry.org/docs/).
