...
Because modules are single files they can easily be shared to others by e.g., email, GitHub, Slack, etc. Another user would simply place the module in their working directory to use it. However, this method of distribution does not scale well in cases of multiple files, if your code depends on additional libraries, or needs a specific version of Python.
...
A "distribution package" (often referred to simply as a "distribution") is a single archive of the Python packages, modules and other files that make up your project. Having a single archive makes it easier to distribute your code to the wild. The base distribution format is called a "source distribution" (`sdist`). An `sdist` is a compressed archive (e.g., `.tar.gz` or `.zip`) of your package. Essentially, an `sdist` provides all of the metadata and source files needed for building and installing your code. You can read more about source distributions [here](https://docs.python.org/3/distutils/sourcedist.html). The standard tool in Python for creating `sdists` (and binary distributions, which we'll explore in the next section) is `setuptools`. 

> Note that, as we saw in [The Whole Game chapter](#whole-game) we prefer to use Poetry (as a simpler and more intuitive alternative to `setuptools`) to create distribution packages of our Python code. We'll discuss Poetry in the later section [Poetry and pyproject.toml](#python-poetry).

As a very simple example, consider the following directory which now contains a `setup.py` file. The `setup.py` file is a standard file that helps `setuptools` build your `sdist` and it is described in detail [here](https://docs.python.org/3/distutils/setupscript.html#).
...
You could also share your `sdist` to PyPI from which a user could install it using `pip install`. It's important to note that installing a package actually adds the package to your default installation directory (more on that in section [Installed packages](#installed-packages)) such that it is accessible outside of your working directory - this is a key difference to simply sharing code as a module or package as we explored in the last two sections. We recommend consulting the [The Hitchhiker's Guide to Packaging](https://the-hitchhikers-guide-to-packaging.readthedocs.io/en/latest/creation.html#) and the [Python docs](https://docs.python.org/3/distutils/sourcedist.html) for more information on creating and distributing source distributions. Some notable examples of Python `sdists` include: [Django](https://github.com/django/django), [hyperlink](https://github.com/python-hyper/hyperlink), [requests](https://github.com/psf/requests).

## Built distribution packages {#built-distributions}

Source distributions are "unbuilt" and require a build step before they can be installed. This nuance is most relevant in cases where your code relies on non-Python code/libraries/packages that require building (aka "compilation") before they can be used (more on that in the next section on [Binary distribution packages](#binary-distributions)). However, even if your package is written in pure Python it still involves a build step to build out the installation metadata. Built distributions are the preferred format for distributing your Python packages. They are packages that have been pre-built and do not require a build step before installation - they only need to be moved to the correct location on your system (we will explore this process more in the section [Installed packages](#installed-packages)). Like a source distribution, a built distribution is a single artefact, and the main built distribution format used by Python is called a `wheel`.

Python's installer `pip` always prefers installing built distributions (`wheels`) over source distributions because installation is always faster. Building `wheels` is similar to building source distributions with `setuptools` as described in the previous section. We won't go into details here because for most users we recommend the use of Python poetry (described later in the section [Poetry and pyproject.toml](#python-poetry)) which handles this build process for you in a simple and intuitive way. However, if you're interested in learning more about using `setuptools` to build a `wheel` of your project we recommend taking a look at the [Python Packaging User Guide tutorial](https://packaging.python.org/tutorials/packaging-projects/).

If your code relies on any non-Python code/libraries/packages, you'll need to use a specific kind of built distribution known as a binary distribution to bundle up your package, which is described in the next section.

## Binary distribution packages {#binary-distributions}

One of the most powerful features of Python is its ability to interoperate with libraries written in other languages, for example, C. Developers sometimes choose to take advantage of this interoperability and include code from other languages in their package to make their code faster, access libraries written in other languages, and generally improve the functionality of their code. While Python is typically referred to as an interpreted language (i.e., your Python code is translated to machine code as it is executed), languages such as C require compilation before they can be used (i.e., your code must be translated into machine code *before* it can be executed). Most end-users will probably not have the tools, experience, or time to build packages containing code written in other languages (typically called "extensions"), so binary distributions are how you make life as easy as possible for installers of your code. Binary distribution packages are simply packages that contains pre-compiled extensions - as an analogy, you can think of your source code as a cake recipe, while a binary distribution is the fully cooked cake.
...
That "single configuration file" is `pyproject.toml` (you can read more about `.toml` files [here](https://www.python.org/dev/peps/pep-0518/)). Essentially, Poetry is based on all the concepts of `sdists` and `wheels` discussed previously - it just simplifies and streamlines the whole packaging process in an intuitive way. In fact, the `poetry build` command you've seen previously, actually creates the `sdist` and `wheel` distributions of your package for you (depending on how you've configured your project).
